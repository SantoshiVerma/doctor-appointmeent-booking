 ...
 const getAvailableSlots =async () =>{
    ...
    for(let i=0;i<7;i++){
      ...
      while(currentDate <endTime){
        ...
        //already book slots so we can not booked this slots
...
const isSlotAvaliable =docInfo.slots_booked[slotDate]
...
if(isSlotAvaliable){
 ...
 const bookAppointment = async () =>{
  ...
  <div className='bg-primary w-full sm:max-w-72 rounded-xl overflow-hidden shadow-md'>
    <img className='w-full h-72 object-contain p-4' src={docInfo.image} />
  </div>
  ...
  <div className='flex-1 border border-gray-300 rounded-xl p-6 bg-white shadow-sm'>
    ...
    <div className='mt-2 text-gray-600 text-sm'>
      <p>
        {docInfo.degree} — <span className='font-medium'>{docInfo.speciality}</span>
      </p>
      <button className='mt-2 bg-green-100 text-green-700 text-xs px-3 py-1 rounded-full'>
        {docInfo.experience}
      </button>
    </div>
